...
const projectsPage = () => {
  ...
  const projectDescs = getContent({
    projectID: "all",
    contentID: 1,
    pageID: "project-hub",
  }) as React.ReactNode[];
  const projectTitles = getContent({
    projectID: "all",
    contentID: 0,
    pageID: "project-hub",
  }) as React.ReactNode[];
    ...
    <>
      ...
      <Section>
        <article className="space-y-3 text-center">
          ...
          <p className="text-xl text-black/70">Here are their achievements.</p>
        </article>
      </Section>
      {/* Card */}
      ...
      <Section className="bg-zinc-200 mt-16">
        ...
        <ul className="flex flex-col gap-8">
          {projectDescs ? (
            projectDescs.map((description, index) => (
              <li
                key={index}
                className="relative grid grid-cols-7 items-center bg-white h-36 px-4 py-4 rounded-3xl pr-20 group overflow-x-hidden"
              >
                ...
                <div
                  className="absolute top-0 right-0 w-full h-full bg-gradient-to-r from-transparent to-asumaroon/30
